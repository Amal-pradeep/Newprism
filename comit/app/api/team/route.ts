...
export async function GET(req:Request){
 ...
 try{const db=await requireAdminDb();
  const [{data:members,error:memberError},{data:templates,error:templateError}]=await Promise.all([
   db.from("team_profiles").select("id,name,email,role,focus,initials,logo_key,color_token,daily_target,automation_lane,active").eq("organization_id",ORG_ID).eq("active",true).order("name"),
