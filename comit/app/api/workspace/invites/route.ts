...
export async function POST(request: Request) {
  ...
  if (member.role !== "owner") return Response.json({ error: "Only the workspace owner can create team passwords." }, { status: 403 });
  ...
  ).bind(known.email.toLowerCase(), known.name, email === member.email ? "owner" : "member", digest(password)).run();
  await db.prepare("DELETE FROM workspace_sessions WHERE email=?").bind(email).run();
  return Response.json({ email: known.email, name: known.name, password }, { headers: { "cache-control": "no-store" } });
}
