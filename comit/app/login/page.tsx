...
export default function Login(){
  ...
  return <main className="grid min-h-screen place-items-center bg-[var(--prism-bg)] p-6"><div className="w-full max-w-md rounded-3xl border border-[var(--prism-border)] bg-[var(--prism-surface)] p-8">
    ...
    <h1 className="text-2xl font-semibold">Welcome back</h1><p className="mt-2 text-sm text-[var(--prism-muted)]">Sign in with your authorized team email. No password required.</p>
    <form onSubmit={submit} className="mt-7 space-y-4">
      ...
      <button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 font-medium text-black">{busy?"Signing in…":<><Mail size={16}/>Continue with email</>}</button>
    </form>
