...
const seed: TeamMember[] = [
  {name:"Amal",role:"Founder / Strategy",focus:"Growth, clients & business intelligence",avatar:"AP",
   tasks:[
  ...
  {name:"Aadil",role:"Business Development",focus:"Prospecting, outreach & follow-ups",avatar:"AS",
   tasks:[
...
export default function TeamPage(){
  ...
  useEffect(()=>{ const saved=localStorage.getItem("comit-team"); if(saved) try{setMembers(JSON.parse(saved))}catch{} },[]);
  ...
  return <main className="min-h-screen p-6 md:p-10">
    <div className="mx-auto max-w-6xl">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div><p className="text-sm text-[var(--prism-blue)]">COMIT · TEAM OS</p><h1 className="mt-1 text-3xl font-semibold">Your team, personalized by AI ✦</h1><p className="mt-2 text-[var(--prism-muted)]">Each profile turns live business signals into a focused task queue.</p></div>
        <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-[var(--prism-muted)]">AI refresh · 120s</div>
      </div>
      ...
      <section className="mt-6 rounded-3xl border border-white/10 bg-white/[.03] p-6">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-wider text-[var(--prism-blue)]">Personal task card</p><h2 className="mt-1 text-xl font-semibold">{member.name}'s priorities</h2></div><span className="rounded-full bg-violet-500/10 px-3 py-1 text-xs">AI personalized</span></div>
        <div className="mt-5 space-y-3">{member.tasks.map(t=><div key={t.title} className="rounded-2xl border border-white/10 bg-black/10 p-4">
          <div className="flex items-start justify-between gap-4"><div><div className="font-medium">{t.title}</div><div className="mt-1 text-xs text-[var(--prism-muted)]">✦ {t.aiReason}</div></div><button onClick={()=>complete(t.title)} className="rounded-lg border border-white/10 px-3 py-1 text-xs hover:bg-white/10">Done</button></div>
          <div className="mt-3 flex gap-2 text-xs"><span className="rounded-full bg-white/5 px-2 py-1">{t.priority}</span><span className="rounded-full bg-white/5 px-2 py-1">{t.due}</span></div>
        </div>)}</div>
      </section>
    </div>
  </main>;
