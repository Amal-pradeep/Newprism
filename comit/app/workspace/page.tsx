...
export default function WorkspacePage() {
  ...
  async function signIn(event: React.FormEvent) {
    ...
    const result = await mutate("/api/workspace/auth", { email, code }, "Signed in to the shared workspace.");
    if (result) setCode("");
  }
  async function signOut() {
    await fetch("/api/workspace/auth", { method: "DELETE" });
    setData(null); setChosen(""); setIssuedCode(""); setMessage("Signed out.");
  }
  ...
  return <main className="min-h-screen bg-[var(--prism-bg)] p-5 pb-24 text-[var(--prism-text)] lg:p-8">
    <div className="mx-auto max-w-7xl">
      ...
      {loading ? <p className="mt-8 text-sm text-[var(--prism-muted)]">Connecting to shared records…</p> : !data ?
        <section className={`${panel} mt-8 max-w-lg`}><div className="flex items-center gap-2 text-violet-300"><LockKeyhole size={18}/><span className="text-xs uppercase tracking-widest">Invite-only access</span></div><h2 className="mt-3 text-xl font-medium">Sign in with your team code</h2><p className="mt-2 text-sm text-[var(--prism-muted)]">Ask Amal for your individual access code. Your session lasts up to 30 days on this device. Keep the code private.</p><form onSubmit={event => void signIn(event)} className="mt-6 space-y-3"><label className="block text-sm">Team email<input className={`${input} mt-2`} type="email" autoComplete="email" required value={email} onChange={event => setEmail(event.target.value)}/></label><label className="block text-sm">Access code<input className={`${input} mt-2`} type="password" autoComplete="off" required value={code} onChange={event => setCode(event.target.value)}/></label><button disabled={busy} className={button}>{busy ? "Checking…" : "Open shared workspace"}</button></form></section>
      : <>
        ...
        {tab === "access" && data.member.role === "owner" && <section className={`${panel} mt-5 max-w-2xl`}><div className="flex items-center gap-2"><ShieldCheck size={18}/><h2 className="text-lg font-medium">Issue teammate access</h2></div><p className="mt-2 text-sm text-[var(--prism-muted)]">Choose a known COMIT teammate. A new code replaces their previous code and signs out their existing sessions. Share it privately; it appears here once.</p><label className="mt-5 block text-sm">Teammate<select className={`${input} mt-2`} value={inviteEmail} onChange={event => {setInviteEmail(event.target.value);setIssuedCode("");}}><option value="">Choose teammate</option>{["aadil.sudhir279@gmail.com","msaneeshnath@gmail.com","jishnu.01010011@gmail.com","shahidruiz01@gmail.com"].map(value => <option key={value} value={value}>{value}</option>)}</select></label><button disabled={busy || !inviteEmail} onClick={() => {setBusy(true);fetch("/api/workspace/invites",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:inviteEmail})}).then(async response => {const result=await response.json();if(!response.ok) throw new Error(result.error || "Could not issue code.");setIssuedCode(result.code);setMessage("New code issued. Share it privately with the selected teammate.");return refresh();}).catch(error => setMessage(error.message)).finally(() => setBusy(false));}} className={`${button} mt-4`}>Create individual code</button>{issuedCode && <div className="mt-5 rounded-xl border border-amber-300/40 bg-amber-500/5 p-4"><p className="text-xs text-amber-200">Copy this code now. It will not be shown again.</p><code className="mt-2 block break-all select-all text-sm">{issuedCode}</code></div>}</section>}
